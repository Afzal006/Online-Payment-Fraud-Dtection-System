...
def upgrade():
    ...
    op.create_table(
        ...
        sa.CheckConstraint("risk_level IN ('LOW', 'MEDIUM', 'HIGH')", name='check_tx_risk_level'),
    ...
    op.create_table(
        ...
        sa.Column('user_id', sa.Integer(), nullable=False),
        sa.Column('alert_type', sa.String(length=50), nullable=False, server_default='FRAUD_ALERT'),
        sa.Column('severity', sa.String(length=20), nullable=False, server_default='HIGH'),
        sa.Column('message', sa.Text(), nullable=False),
        sa.Column('status', sa.String(length=20), nullable=False, server_default='OPEN'),
        sa.Column('created_at', sa.DateTime(), nullable=False),
        sa.Column('resolved_at', sa.DateTime(), nullable=True),
        sa.CheckConstraint("severity IN ('MEDIUM', 'HIGH', 'CRITICAL')", name='check_alert_severity'),
        sa.CheckConstraint("status IN ('OPEN', 'RESOLVED', 'DISMISSED')", name='check_alert_status'),
